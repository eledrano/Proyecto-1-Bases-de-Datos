...
class AppAgenda(ctk.CTk):
    ...
    def crear_sidebar(self):
        ...
        for i, (nombre, icono) in enumerate([
            ...
            ("Categorías", "📁"),
            ("Eventos", "🗓️"),
            ("Ubicaciones", "📍"), # Crear pestaña Ubicaciones
            ("Tareas", "📝"), # Crear pestaña Tareas
            ("Disponibilidad", "🕰️"), # Crear pestaña Disponibilidad
        ], start=2):
            btn = ctk.CTkButton(
    ...
    def crear_area_principal(self):
        ...
        self.configurar_pestana_ubicaciones() # Ampliación area de Ubicaciones en la interfaz
        self.configurar_pestana_tareas()  # Ampliación area de Tareas en la interfaz
        self.configurar_pestana_disponibilidad()  # Ampliación area de Disponibilidad en la interfaz
    ...
    def configurar_pestana_eventos(self):
        ...
        ctk.CTkLabel(form, text="Ubicación").pack(anchor="w", padx=10, pady=(10, 2))
    ...
    def cargar_evento_seleccionado(self, _=None):
        ...
        self.combo_ev_ubicacion.set(vals[6])
        try:
    ...
    def cargar_datos_eventos(self):
        # Ampliar ubicacion en SELECT, crear un JOIN adicional y ubicacion dentro de ciclo for
        try: 
            rows = self.ejecutar_consulta("""
    ...
    def configurar_pestana_ubicaciones(self):
        self.crear_encabezado(self.tab_ubicaciones, "Ubicaciones", "Gestiona los recintos físicos en los que se realizan eventos")
        ...
        ctk.CTkButton(form, text="💾 Actualizar selección", command=self.actualizar_ubicacion).pack(fill="x", padx=10, pady=5)
        ctk.CTkButton(form, text="🧹 Nueva / Limpiar", command=self.limpiar_form_ubicacion, fg_color="gray").pack(fill="x", padx=10, pady=5)
        ctk.CTkButton(form, text="🗑️ Eliminar selección", command=self.eliminar_ubicacion, fg_color="#b33939", hover_color="#8f2d2d").pack(fill="x", padx=10, pady=5)
    ...
    def cargar_ubicacion_seleccionada(self, _=None):
        ...
        self.entry_ub_direccion.delete(0, tk.END); self.entry_ub_direccion.insert(0, vals[2])
        self.entry_ub_ciudad.delete(0, tk.END); self.entry_ub_ciudad.insert(0, vals[3])
    ...
    def datos_ubicacion_formulario(self):
        ...
        if not nombre or not direccion or not ciudad or not capacidad:
            raise ValueError("Complete nombre, dirección, ciudad y capacidad.")
        if not capacidad.isdigit() or int(capacidad) <= 0:
            raise ValueError("Capacidad debe ser un número entero mayor que 0")
    ...
    def cargar_datos_ubicaciones(self):
        try:
            rows = self.ejecutar_consulta(
                "SELECT id_ubicacion, nombre, direccion, ciudad, capacidad FROM ubicaciones ORDER BY nombre", fetch=True)
    ...
    def configurar_pestana_tareas(self):
        self.crear_encabezado(self.tab_tareas, "Tareas", "Administra labores y actividades a realizar para un evento")
        ...
        self.entry_ta_titulo = ctk.CTkEntry(form, placeholder_text="Título de la tarea")
        self.entry_ta_titulo.pack(fill="x", padx=10, pady=6)
            
        self.entry_ta_descripcion = ctk.CTkEntry(form, placeholder_text="Decripción")
        self.entry_ta_descripcion.pack(fill="x", padx=10, pady=6)

        ctk.CTkLabel(form, text="Evento").pack(anchor="w", padx=10, pady=(8, 2))
        self.combo_ta_evento = ctk.CTkComboBox(form, values=["Seleccione un evento"], state="readonly")
        self.combo_ta_evento.set("Seleccione un evento")
        self.combo_ta_evento.pack(fill="x", padx=10, pady=4)

        ctk.CTkLabel(form, text="Responsable").pack(anchor="w", padx=10, pady=(8, 2))
        self.combo_ta_usuario = ctk.CTkComboBox(form, values=["Seleccione un usuario"], state="readonly")
        self.combo_ta_usuario.set("Seleccione un usuario")
        self.combo_ta_usuario.pack(fill="x", padx=10, pady=4)

        ctk.CTkLabel(form, text="Prioridad").pack(anchor="w", padx=10, pady=(8, 2))
        self.combo_ta_prioridad = ctk.CTkComboBox(form, values=["Alta", "Media", "Baja"], state="readonly")
        self.combo_ta_prioridad.set("Media")
        self.combo_ta_prioridad.pack(fill="x", padx=10, pady=4)

        ctk.CTkLabel(form, text="Estado").pack(anchor="w", padx=10, pady=(8, 2))
        self.combo_ta_estado = ctk.CTkComboBox(form, values=["Pendiente", "En progreso", "Completada", "Cancelada"], state="readonly")
        self.combo_ta_estado.set("Pendiente")
        self.combo_ta_estado.pack(fill="x", padx=10, pady=4)

        ctk.CTkLabel(form, text="Fecha Límite").pack(anchor="w", padx=10, pady=(8, 2))
        self.fecha_limite = self.crear_selector_fecha(form)
        self.fecha_limite.pack(fill="x", padx=10, pady=4)

        ctk.CTkButton(form, text="➕ Crear tarea", command=self.agregar_tarea).pack(fill="x", padx=10, pady=(15, 5))
        ctk.CTkButton(form, text="💾 Actualizar selección", command=self.actualizar_tarea).pack(fill="x", padx=10, pady=5)
        ctk.CTkButton(form, text="🧹 Nueva / Limpiar", command=self.limpiar_form_tarea, fg_color="gray").pack(fill="x", padx=10, pady=5)
        ctk.CTkButton(form, text="🗑️ Eliminar selección", command=self.eliminar_tarea, fg_color="#b33939", hover_color="#8f2d2d").pack(fill="x", padx=10, pady=5)
    ...
    def cargar_tarea_seleccionada(self, _=None):
        ...
        self.entry_ta_titulo.delete(0, tk.END); self.entry_ta_titulo.insert(0, vals[1])
        self.combo_ta_evento.set(vals[2])
        self.combo_ta_usuario.set(vals[3])
        self.combo_ta_prioridad.set(vals[4])
        self.combo_ta_estado.set(vals[5])
        if vals[6]:
            self.establecer_fecha(self.fecha_limite, vals[6])

    def limpiar_form_tarea(self):
        self.tree_tareas.selection_remove(self.tree_tareas.selection())
        self.entry_ta_titulo.delete(0, tk.END)
        self.entry_ta_descripcion.delete(0, tk.END)
        self.combo_ta_evento.set("Seleccione un evento")
        self.combo_ta_usuario.set("Seleccione un usuario")
        self.combo_ta_prioridad.set("Media")
        self.combo_ta_estado.set("Pendiente")
        self.establecer_fecha(self.fecha_limite, datetime.now())

    def datos_tarea_formulario(self):
        titulo = self.entry_ta_titulo.get().strip()
        descripcion = self.entry_ta_descripcion.get().strip()
        evento = self.eventos_combo.get(self.combo_ta_evento.get())
        usuario = self.usuarios_combo.get(self.combo_ta_usuario.get())
        prioridad = self.combo_ta_prioridad.get()
        estado = self.combo_ta_estado.get()
        fecha_limite = self.obtener_fecha(self.fecha_limite)
        if not titulo or evento is None or usuario is None:
            raise ValueError("Complete título, evento y responsable.")
        return evento, usuario, titulo, descripcion, prioridad, estado, fecha_limite
    ...
    def agregar_tarea(self):
        try:
            ...
            self.ejecutar_consulta("""
                INSERT INTO tareas
                (id_evento, id_usuario, titulo, descripcion, prioridad, estado, fecha_limite)
    ...
    def actualizar_tarea(self):
        ...
        if tid is None: return messagebox.showwarning("Selección requerida", "Seleccione una tarea.")
        try:
            evento, usuario, titulo, descripcion, prioridad, estado, fecha_limite = self.datos_tarea_formulario()
            self.ejecutar_consulta("""
                UPDATE tareas SET id_evento=%s, id_usuario=%s, titulo=%s, descripcion=%s, prioridad=%s, estado=%s, fecha_limite=%s
                WHERE id_tarea=%s
            """, (evento, usuario, titulo, descripcion, prioridad, estado, fecha_limite, tid))
    ...
    def cargar_datos_tareas(self):
        try:
            rows = self.ejecutar_consulta("""
                SELECT t.id_tarea, t.titulo, e.id_evento, e.titulo, u.id_usuario, u.nombre, u.apellido, t.prioridad, t.estado, t.fecha_limite
                FROM tareas t
            ...
            for row in rows:
                evento = f"{row[3]} - #{row[2]}"
                usuario = f"{row[5]} {row[6]} - #{row[4]}"
                limite = row[9].strftime("%Y-%m-%d") if hasattr(row[9], "strftime") else row[9]
                self.tree_tareas.insert("", "end", values=(row[0], row[1], evento, usuario, row[7], row[8], limite))
            valores_e = ["Seleccione un evento"] + list(self.eventos_combo.keys())
            valores_u = ["Seleccione un usuario"] + list(self.usuarios_combo.keys())
            self.combo_ta_evento.configure(values=valores_e)
    ...
    def configurar_pestana_disponibilidad(self):
        self.crear_encabezado(self.tab_disponibilidad, "Disponibilidad", "Revisar las franjas horarias y su disponibilidad por usuario")
        ...
        self.combo_di_usuario = ctk.CTkComboBox(form, values=["Seleccione un usuario"], state="readonly")
        self.combo_di_usuario.set("Seleccione un usuario")
        self.combo_di_usuario.pack(fill="x", padx=10, pady=4)

        ctk.CTkLabel(form, text="Tipo").pack(anchor="w", padx=10, pady=(8, 2))
        self.combo_di_tipo = ctk.CTkComboBox(form, values=["Seleccione un tipo"], state="readonly")
        self.combo_di_tipo.set("Seleccione un tipo")
        self.combo_di_tipo.pack(fill="x", padx=10, pady=4)
        ...
        self.hora_di_inicio = ctk.CTkEntry(form, placeholder_text="HH:MM")
        self.hora_di_inicio.pack(fill="x", padx=10, pady=4)

        ctk.CTkLabel(form, text="Hora Fin").pack(anchor="w", padx=10, pady=(8, 2))
        self.hora_di_fin = ctk.CTkEntry(form, placeholder_text="HH:MM")
        self.hora_di_fin.pack(fill="x", padx=10, pady=4)        

        ctk.CTkButton(form, text="➕ Crear", command=self.agregar_disponibilidad).pack(fill="x", padx=10, pady=(15, 5))
        ctk.CTkButton(form, text="💾 Actualizar selección", command=self.actualizar_disponibilidad).pack(fill="x", padx=10, pady=5)
        ctk.CTkButton(form, text="🧹 Nueva / Limpiar", command=self.limpiar_form_disponibilidad, fg_color="gray").pack(fill="x", padx=10, pady=5)
        ctk.CTkButton(form, text="🗑️ Eliminar selección", command=self.eliminar_disponibilidad, fg_color="#b33939", hover_color="#8f2d2d").pack(fill="x", padx=10, pady=5)
    ...
    def cargar_disponibilidad_seleccionada(self, _=None):
        ...
        self.combo_di_usuario.set(vals[1])
        self.combo_di_tipo.set(vals[2])  
        self.establecer_fecha(self.fecha_disponibilidad, vals[3])
        self.hora_di_inicio.delete(0, tk.END); self.hora_di_inicio.insert(0, vals[4])          
        self.hora_di_fin.delete(0, tk.END); self.hora_di_fin.insert(0, vals[5])  
    ...
    def limpiar_form_disponibilidad(self):
        ...
        self.hora_di_inicio.delete(0, tk.END); self.hora_di_inicio.insert(0, "08:00")
        self.hora_di_fin.delete(0, tk.END); self.hora_di_fin.insert(0, "12:00")

    def datos_disponibilidad_formulario(self):
        usuario = self.usuarios_combo.get(self.combo_di_usuario.get())
        tipo = self.tipos_combo.get(self.combo_di_tipo.get())
        fecha = self.obtener_fecha(self.fecha_disponibilidad)
        hora_inicio = self.hora_di_inicio.get().strip()
        hora_fin = self.hora_di_fin.get().strip()
        if usuario is None or tipo is None:
            raise ValueError("Seleccione un usuario y tipo de disponibilidad")
        try:
            ini = datetime.strptime(hora_inicio, "%H:%M")
            fin = datetime.strptime(hora_fin, "%H:%M")
        except ValueError:
            raise ValueError("Las horas deben ser en el formato HH:MM")
        if fin <= ini:
            raise ValueError("Hora fin debe ser posterior que Hora inicio")
        return usuario, tipo, fecha, hora_inicio, hora_fin
    ...
    def agregar_disponibilidad(self):
        try:
            ...
            self.ejecutar_consulta("""
                ...
                (id_usuario, id_tipo, fecha, hora_inicio, hora_fin)
                VALUES (%s, %s, %s, %s, %s)
            """, datos)
            self.limpiar_form_disponibilidad(); self.cargar_datos_disponibilidad()
            messagebox.showinfo("Éxito", "Franja horaria creada correctamente.")
        except Exception as e:
            messagebox.showerror("No se pudo crear la franja horaria", str(e))

    def actualizar_disponibilidad(self):
        did = self.disponibilidad_seleccionada_id()
        if did is None: return messagebox.showwarning("Selección requerida", "Seleccione una franja horaria.")
        try:
            usuario, tipo, fecha, hora_inicio, hora_fin = self.datos_disponibilidad_formulario()
            self.ejecutar_consulta("""
                UPDATE disponibilidades SET id_usuario=%s, id_tipo=%s, fecha=%s, hora_inicio=%s, hora_fin=%s
                WHERE id_disponibilidad=%s
            """, (usuario, tipo, fecha, hora_inicio, hora_fin, did))
            self.cargar_datos_disponibilidad()
            messagebox.showinfo("Éxito", "Franja horaria actualizada.")
        except Exception as e:
            messagebox.showerror("No se pudo actualizar", str(e))
    ...
    def eliminar_disponibilidad(self):
        ...
        try:
            ...
            self.limpiar_form_disponibilidad(); self.cargar_datos_disponibilidad()
            messagebox.showinfo("Eliminado", "Franja horaria eliminada.")
        except Exception as e:
            messagebox.showerror("No se pudo eliminar", str(e))
    ...
    def cargar_datos_disponibilidad(self):
        try:
            ...
            for tid, nombre in tipos:
                self.tipos_combo[f"{nombre} - #{tid}"] = tid
            rows = self.ejecutar_consulta("""
                SELECT d.id_disponibilidad, u.id_usuario, u.nombre, u.apellido, td.id_tipo, td.nombre, d.fecha, d.hora_inicio, d.hora_fin
            ...
            for row in rows:
                ...
                ini = row[7].strftime("%H:%M") if hasattr(row[7], "strftime") else str(row[7])[:5]
                fin = row[8].strftime("%H:%M") if hasattr(row[8], "strftime") else str(row[8])[:5]
                self.tree_disponibilidad.insert("", "end", values=(row[0], usuario, tipo, fecha, ini, fin))
            valores_u = ["Seleccione un usuario"] + list(self.usuarios_combo.keys())
            valores_t = ["Seleccione un tipo"] + list(self.tipos_combo.keys())
            self.combo_di_usuario.configure(values=valores_u)
            self.combo_di_tipo.configure(values=valores_t)
        except Exception as e:
